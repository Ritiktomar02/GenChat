...
export const resetPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.error("Reset success email failed:", emailError);
      }
    }
...
export const googleLogin = async (req, res) => {
  try {
    ...
    const { tokens } = await oauth2Client.getToken({ code, redirect_uri: redirectUri || "postmessage" });
    ...
    if (!user) {
      user = await User.create({
        username: name,
        email,
        picture: picture || generateAvatar(name),
        authProvider: "google",
        isVerified: true,
      });
    }
